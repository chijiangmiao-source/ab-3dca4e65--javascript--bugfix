...
- **任意精度代价**：代价可为任意合法非负整数（含超过 `2^53−1` 者）。页面从
  文本录入、请求提交（BigInt 逐位序列化）到响应解析与十进制展示全程不经过
  丢精度的 `Number`；大整数不会仅因超出安全整数范围而被拒绝。
...
verify/              一次性核对服务
  ...
  page_pipeline.mjs    驱动页面真实模块，核对录入/提交/展示管线的大整数精度
