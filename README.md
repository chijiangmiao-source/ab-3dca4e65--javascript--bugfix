...
- **容器化**：Web 与 API 各自独立 Dockerfile，Docker Compose 编排，
  ...
  其中包含 `2^53` 相邻大整数（`9007199254740993` / `9007199254740992`）
  边界：复用页面源码走「文本录入 → 请求提交 → 响应处理与格式化」全链路，
  断言两条平行通道经页面提交与精确整数直连 API 结果一致、规范通道为更便宜者、
  总代价与逐边代价在页面中仍显示完整十进制。
...
标识唯一、代价为任意非负整数（无上限，页面与 API 均保留超过 `2^53−1`
的完整整数精度，不会因超出 JavaScript 安全整数范围而被拒绝或舍入）、
允许平行通道、禁止自环。
